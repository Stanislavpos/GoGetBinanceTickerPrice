...
func pricePair(pair string) {
	...
	case "BTCUSDT":
		...
		btcusdtToSQL(priceInt)
	case "ETHUSDT":
		priceFloat = priceFloat * 100
		priceInt := int(priceFloat)
		ethusdtToSQL(priceInt)
	case "RVNUSDT":
		priceFloat = priceFloat * 100000000
		priceInt := int(priceFloat)
		rvnusdtToSQL(priceInt)
	}
...
func btcusdtToSQL(priceInt int) {
	connStr := "user=postgres password=123-123-123-123 dbname=postgres sslmode=disable"
...
func rvnusdtToSQL(priceInt int) {
	connStr := "user=postgres password=123-123-123-123 dbname=postgres sslmode=disable"
	db, err := sql.Open("postgres", connStr)
	if err != nil {
		panic(err)
	...
	_, err = db.Exec("insert into rvnusdt (price, date) values ($1, $2)", priceInt, time.Now())
	if err != nil {
		panic(err)
	}
...
func ethusdtToSQL(priceInt int) {
	connStr := "user=postgres password=123-123-123-123 dbname=postgres sslmode=disable"
	db, err := sql.Open("postgres", connStr)
	if err != nil {
		panic(err)
	...
	_, err = db.Exec("insert into ethusdt (price, date) values ($1, $2)", priceInt, time.Now())
	if err != nil {
		panic(err)
	}
